...
- (iii) one-sidedness of the guard: a deterministic conditional statement on the guard event $\{[\bar{\hat v^2}-z\,SE]_+\le\bar v^2\ \forall t\}$ about the *design-variance-subtraction component* only ($\hat s_T^2\ge\max\{\hat s_{\rm plug}^2-\bar v^2,\text{floor}\}$); the sampling error of $\hat s_{\rm plug}^2$ stays two-sided and is priced by (ii). $z=1.645$ is a normal-reference operational margin with no theorem-level probability attached (A3 gives Bernstein concentration, not a normal pivot). Statement and proof now say the same thing.
