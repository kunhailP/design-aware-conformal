...
| Generative-AI policy: code/data-analysis use disclosed in methods; text/proof revision in Acknowledgments; cover-letter mention; tool, version, dates, access, scope | ✅ | **Written 2026-09-07** in all three slots: end of §3 (code/data analysis), Acknowledgments (text/proofs), cover letter (one sentence); each names tool/version, June–September dates, Claude.ai access, scope, and author verification. No placeholders remain in `paper/main.tex`; the ledger test `test_no_author_placeholders_left` passes. Earlier drafts: `git show b28efc9:paper/main.tex`. |
...
| Reproducibility verifiable by the PA team | ✅ | contract tests + claim ledger (**135 passing at the current main, CI green**); e13/e26/e50 verified bit-identical from raw files in two environments |
