...
with the true target scale is finite-sample valid for the latent target,
exact at the attainable conformal rank, and its target scale is a factor
$\sqrt{1-\rho(t)^2}$ of the observed scale at each coordinate --- the
realized width carries in addition the ratio of the two conformal quantiles,
which tends to one as $\rho\to0$; some such shape restriction is necessary
...
Material) the scale error can only widen the band. Some shape restriction is not removable (the
