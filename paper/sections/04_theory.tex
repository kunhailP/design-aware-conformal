...
Theorem~\ref{thm:exact} anchors validity and Theorem~\ref{thm:safe}
carries it through the selector; Theorems~\ref{thm:oracle}
and~\ref{thm:estimated} price the latent target and the estimated noise law. Proofs are in the
...
vanishes with the design-noise scale. Since $|\xi|\le\lVert S\rVert_\infty$,
the tail term concerns the design error alone; with $\sigma_\xi$ a scale for
$\xi$, the gap is of order $\sigma_\xi^{2/3}$ under variance-only tails and
of order $\sigma_\xi$ up to logarithms under sub-Gaussian noise. The gap
does not vanish by symmetry alone (a bimodal counterexample is in the
Supplementary Material), and nothing assumes $\xi$ centred. Hence we estimate
and report the design share rather than assume it away.
...
bounded curves with an unbiased size-$B$ design bootstrap (A3); the
Supplementary Material states all three. Then (i)~with oracle scales the deconvolution band
...
two-sided and is absorbed into $\varepsilon_{K,B}$.
...
$K\approx30$; the frozen gates decide on the uninflated radii, so the sealed
validation carries over a fortiori. The exact split-fold alternative is infeasible at survey scale:
...
cannot inflate miscoverage (Theorem~\ref{thm:safe}). Remaining width
results, one open, are in the Supplementary Material.
