...
The literature's Greek legitimacy collapse
\citep{armingeon2014democracy,torcal2014decline} concerns 2009--2015; the
certified pair is 2020/21$\to$2023/24.
...
rounds 1--8 weights-only, the sample-design-file merge changing no net or
persistent count --- Supplementary Material)
...
magnitude differ: Spain certifies $32$ of $55$ contrasts yet clears its span
by only $0.6$ CDF points. Ukraine's certified span decline of at least $25$ points runs
...
\emph{inconclusive under the design-aware band}, not ``false positives,'' a
term we reserve for simulation with known truth.
...
$\times2.0$ the core holds at $13$ and $12$ countries and the gap
...
co-certification on two separate $\alpha=0.10$ bands, not a $90\%$ statement
that two items truly declined. The core is
...
than the marginal flag, Fisher $p=0.89$;
\citealp{ouattara2023distrusting,vandermeer2025pushing}): certification is
descriptive error control, not a forecast.
