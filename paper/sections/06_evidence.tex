...
correction \emph{unnecessary}, \emph{unlearnable}, \emph{feasible} --- and
this paper's datasets occupy the three corners.
...
bounds of $0.16$--$0.22$ across twelve independent bootstrap streams.
...
\paragraph{How coarse can the calibration unit be?} One constraint is
sharp: the conformal level is infeasible whenever
$K<\alpha^{-1}-1$, the required order statistic exceeding the sample
(Theorem~\ref{thm:exact}'s infinite-radius convention) --- a too-coarse
calibration unit is unusable, not merely inefficient --- while refining is
not free either, half-width falling threefold as units coarsen
(Supplementary Material). The estimand's unit is fixed by the question
\citep{openshaw1984modifiable,robinson1950ecological}.
