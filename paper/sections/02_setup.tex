...
leave-one-out centering, itself theorem-covered after the $K/(K{-}1)$
radius inflation the released default applies
...
primary-sampling-unit (PSU) / replicate-weight bootstrap can estimate. Define the transport score of a country as its worst-case
...
shares the anchor's row, containing it by construction; the certification
band is design-based and within-country, and the theory's finite-sample
language does not transfer to it.}\label{tab:targets}
\end{table}
...
The reading is constructive (proof in the Supplementary Material, Section~S1): a band that is both valid
...
uses estimated objects: model-based small-area outputs, learned or imputed features,
