...
distribution declined simultaneously across the trajectory (a Bonferroni
layer across countries strengthens along a different axis and sits atop any
rung). The claims form a partial
...
cost we report (Section~\ref{sec:political}). The persistent object is covered directly by one functional band
...
--- $z\,\SE$ a one-sided margin on the mean design variance at guard level
$\alpha_2$ ($z=1.645$ deployed), the floor a small positive constant --- which
subtracts a \emph{lower} confidence bound on the design variance so the
correction cannot over-shrink; its width scales as $\sqrt{1-\rho^2}$ relative
to the clustered band. \emph{(iii) Conservative} inflates each unstudentized score by the design SD,
...
$D=\max_t \SE(\widehat s_T^2)/\widehat s_T^2$; equivalently $D\le\tau_D$
under the frozen affine $\widehat\delta_{\text{UCB}}$,
Proposition~\ref{prop:unreach}), \emph{efficiency} (a certified width gain
$\widehat{\text{Gain}}$ over the conservative band clearing a margined
threshold), and \emph{stability} (the guarded subtraction leaves the scale
above its positive floor); otherwise the band is an anchor, as
Algorithm~\ref{alg:safe} routes. Every gate is target-blind, so by
...
\KwIn{calibration curves $\{\widetilde E_c\}$ (unit = country), design replicates
